...
touch container-minimal.img
truncate -s 4G container-minimal.img

losetup -D
losetup -fP --show container-minimal.img
...
pacstrap -C ./archlinux.conf -K /mnt \
                ...
                docker docker-compose linux dhcpcd openssh e2fsprogs dosfstools \
...
default container.conf
EOT

cat <<'EOT' > /boot/loader/entries/container.conf
title   Container VM
linux   /Image
...
# uid 1000
useradd -m -G wheel,docker,video,audio,storage,power -s /usr/bin/zsh user
passwd -d user
...
ln -s /usr/local/lib/docker /var/lib/docker
...
[Service]
ExecStartPre=/usr/bin/mkdir -p /usr/local/lib/docker
EOT
...
# lzfse -encode -i container-minimal.img -o ../container-minimal.img.lzfse
xz -vkc --format=lzma -9e container-minimal.img > ../container-minimal.img.lzma
